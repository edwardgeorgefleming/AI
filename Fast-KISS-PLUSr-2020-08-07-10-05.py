...
alpha = float(input('Enter a value between 1.0 and 0.0 for the accuracy in which you would like the program to test at (1.0 = 100%). '))
...
for k in range(20):
    ...
    for ii in range(1 << iin):
        ...
        X = A[1] and A[4] and not A[2] or A[7] or not A[49] or A[343]
        ...
        if L:
            ...
            for j in range(nj1):
                ...
                if TA[j2] >= TT and TnA[j2] >= TT and TX >= TT and TnX >= TT:
                    if TnAnX[j2] >= TnA[j2] * alpha:
                        op[j] = 1
                        label_not[j] = 0
                    if TAnX[j2] >= TA[j2] * alpha:
                        op[j] = 1
                        label_not[j] = 1
                     
                    if TAX[j2] >= TA[j2] * alpha:
                        op[j] = 2
                        label_not[j] = 0
                    if TnAX[j2] >= TnA[j2] * alpha:
                        op[j] = 2
                        label_not[j] = 1
                else:
                    overfit = True
