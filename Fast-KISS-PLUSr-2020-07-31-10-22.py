...
for k in range(20):
    ...
    for ii in range(1 << iin):
        ...
        if L:
            ...
            for j in range(nj1):
                op[j] = 0
                j2 = j1[j]
                if TA[j2] > 0 and TnA[j2] > 0 and TX > 0 and TnX > 0:
                    if TnAnX[j2] == TnA[j2]:
                        op[j] = 1
                        label_not[j] = 0
                    if TAnX[j2] == TA[j2]:
    ...
    if Te != 0:
        ...
        print(' Seconds.')
        print('Efficiency of Correct Outputs = ', e , end = '')
        print(' / ', Te, end = '')
        print(' = ',e/Te)
        print()
    ...
    answer = input('Do you want to go to the SUPER HIGH SPEED in the "AS TAUGHT MODE"  (y or n)? ')
    if answer == 'y':
        L = False
    else:
        L = True
